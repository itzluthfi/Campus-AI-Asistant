...
const FileDownloadCard: React.FC<{ file: GeneratedFile }> = ({ file }) => {
  const handleDownload = () => {
    // 1. Handle PDF Generation (Client-Side)
    if (file.mimeType === 'application/pdf') {
      try {
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            data = parsed;
             ...
             // Jika object tunggal, jadikan array
             data = [parsed];
          }
        } catch (e) {
          parsingError = true;
        ...
        doc.setFontSize(16);
        doc.text("Laporan Kampus (Generated by AI)", 14, 20);
        doc.setFontSize(10);
        doc.text(`File: ${file.filename}`, 14, 28);
        doc.text(`Tanggal Cetak: ${new Date().toLocaleDateString('id-ID')}`, 14, 33);
        
        // JIKA DATA KOSONG ATAU ERROR PARSING
        if (parsingError || data.length === 0) {
           doc.setFontSize(11);
           doc.setTextColor(150, 0, 0);
           doc.text("Pemberitahuan:", 14, 45);
           doc.setFontSize(10);
           doc.setTextColor(0, 0, 0);
           
           if (parsingError) {
             // Jika AI mengirim teks biasa, bukan JSON, tampilkan teksnya
             const splitText = doc.splitTextToSize(file.content, 180);
             doc.text(splitText, 14, 52);
           } else {
             doc.text("Tidak ada data yang tersedia untuk laporan ini berdasarkan filter pencarian Anda.", 14, 52);
           }
        } else {
          // JIKA DATA ADA
          const headers = Object.keys(data[0]).map(k => k.toUpperCase().replace(/_/g, ' '));
          const rows = data.map((obj: any) => Object.values(obj).map(val => String(val)));
        ...
        doc.save(file.filename.endsWith('.pdf') ? file.filename : `${file.filename}.pdf`);
      } catch (e) {
        console.error("PDF Generation Error:", e);
        alert("Gagal generate PDF. Format data dari AI tidak kompatibel.");
      }
      return;
    }

    // 2. Handle CSV (Add BOM for Excel)
    ...
    // 3. Handle Word (HTML to Doc trick)
    if (file.mimeType === 'application/msword') {
      const preHtml = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Export HTML To Doc</title></head><body>";
      const postHtml = "</body></html>";
      const html = preHtml + file.content + postHtml;
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = file.filename.endsWith('.doc') ? file.filename : `${file.filename}.doc`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      return;
  ...
  const getIconColor = () => {
    if (file.mimeType === 'application/pdf') return 'bg-red-100 text-red-600';
    if (file.mimeType === 'application/msword') return 'bg-blue-100 text-blue-600';
    if (file.mimeType === 'text/csv') return 'bg-green-100 text-green-600';
    return 'bg-gray-100 text-gray-600';
  }

  const getLabel = () => {
    if (file.mimeType === 'application/pdf') return 'PDF Document';
    if (file.mimeType === 'application/msword') return 'Word Document';
    if (file.mimeType === 'text/csv') return 'Excel / CSV';
    return 'Text File';
  }

  return (
    <div className={`mt-3 p-3 bg-white border-l-4 rounded shadow-sm flex items-center justify-between max-w-sm ${file.mimeType === 'application/pdf' ? 'border-red-500' : 'border-green-500'}`}>
      <div className="flex items-center gap-3">
         <div className={`p-2 rounded ${getIconColor()}`}>
      ...
      <button 
        ...
        className="px-3 py-1 bg-gray-800 text-white text-xs font-semibold rounded hover:bg-gray-700 transition"
      >
        Download
      </button>
    </div>
