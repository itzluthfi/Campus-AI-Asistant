...
// Komponen Visualisasi Data Sederhana (Custom CSS Chart)
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ database, user, setUser }) => {
  ...
  const [loadingLogs, setLoadingLogs] = useState<string[]>([]); // Realtime logs
      ...
      setMessages([
        {
          id: 'init',
          role: 'model',
          text: `Halo! Saya AI Asistant Kampus Cerdas.\n\nFitur Baru:\n1. 📊 **Analisis Statistik** (Coba: "Tampilkan grafik jumlah mahasiswa per jurusan")\n2. 👁️ **Visi Komputer** (Kirim foto jadwal/dokumen untuk saya baca)\n\nKetik "login" untuk akses data.`,
          timestamp: new Date()
        }
      ]);
    }
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if (!user) {
      // Logic Login yang sama seperti sebelumnya (Simplified for brevity as it was working)
      if (authStep === 'IDLE' && userText.toLowerCase() === 'login') {
        setAuthStep('ASK_ID');
        setIsLoading(false);
        setTimeout(() => addBotMessage("Masukkan NIM, NIP, atau Username Admin:"), 500);
        return;
      }
      if (authStep === 'ASK_ID') {
        setTempId(userText);
      ...
      if (authStep === 'ASK_PASSWORD') {
        ...
        if (foundUser) {
          const newUserSession: UserSession = {
            ...
            role: role,
            name: foundUser.name,
            identifier: role === 'student' ? foundUser.nim : (role === 'lecturer' ? foundUser.nip : foundUser.username)
          };
          ...
          setTempId('');
          setIsLoading(false);
          setTimeout(() => addBotMessage("❌ Login Gagal. Username/Password salah. Ketik 'login' untuk coba lagi."), 500);
        }
        return;
      }
    }
    ...
    try {
      if (authStep === 'IDLE') {
        ...
        const result = await generateAIResponse(
          ...
          user, 
          userImage || undefined,
          (log) => setLoadingLogs(prev => [...prev, log]) // Update logs realtime
        );
    ...
    <div className="flex flex-col h-full bg-[#e5ddd5]">
      ...
      <div className="bg-[#075e54] p-3 flex items-center justify-between shadow-md z-10 text-white">
        <div className="flex items-center gap-3">
          <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold ${user?.role === 'admin' ? 'bg-red-500 text-white' : 'bg-white text-[#075e54]'}`}>
          ...
          <div>
            ...
            <p className="text-green-100 text-xs">
              {user ? (user.role === 'student' ? 'Mahasiswa' : (user.role === 'admin' ? 'Administrator' : 'Dosen')) : 'Mode Terbatas'}
            </p>
          </div>
        </div>
        {!user && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4" style={{ backgroundImage: 'url("https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png")', backgroundRepeat: 'repeat' }}>
        ...
        {/* Loading Indicator with Realtime Logs */}
        {isLoading && (
          <div className="bg-white rounded-lg rounded-tl-none px-4 py-3 shadow-sm w-fit max-w-[80%]">
             <div className="flex items-center space-x-2 mb-2">
               <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
               <span className="text-xs font-semibold text-gray-600">AI sedang memproses...</span>
             </div>
             <div className="text-[10px] font-mono text-gray-500 space-y-1 border-l-2 border-gray-200 pl-2">
               ...
               <div className="w-1.5 h-4 bg-gray-400 animate-pulse inline-block align-middle ml-1"></div>
             </div>
          </div>
        )}
          ...
          <input
            ...
            placeholder={
              authStep === 'ASK_PASSWORD' 
                ? "Masukkan Password..." 
                : (user ? "Ketik pesan..." : "Tanya sesuatu atau kirim gambar")
            }
